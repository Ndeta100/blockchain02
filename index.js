const $={
    blocks:[],
    init:async function(){
        console.log('ADECOIN has initialized')
        await $.createGenesis()
        await $.addBlock('This is a new block')
        console.log($.blocks)
    },
    createGenesis:async function(){
       
        const block={
            message:'ADECOIN is the next UNICORN crypto going to the moon',
            dateTime:Date.now(),
            prev_hash:0,
            id:0,
        }
    ...
    addBlock:async function(message){
        let prev_hash= await $.getprevHash()
